...
class ImageClassificationInferencer100(BaseInferencer):
    """The inferencer for image classification.
    ...
    def postprocess(self,
        ...
        for data_sample in preds:
            ...
            # 获取最大的100个值及其对应的索引
            topk_values, topk_indices = torch.topk(pred_scores, k=100)
            ...
            if self.classes is not None:
                result['pred_class'] = self.classes[pred_label]
                # 获取最大的100个值对应的类别和分数
                top100_classes = [self.classes[idx] for idx in topk_indices.cpu().numpy()]
                top100_scores = topk_values.cpu().numpy().tolist()
                
                result['top100_classes'] = top100_classes
                result['top100_scores'] = top100_scores
                
                # 也可以选择将类别和分数组合成字典形式
                # result['top100_predictions'] = dict(zip(top100_classes, top100_scores))
