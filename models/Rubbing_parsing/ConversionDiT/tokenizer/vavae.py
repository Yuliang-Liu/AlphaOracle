...
class VA_VAE:
    ...
    def __init__(self, config, device, horizon_flip=0.5, fp16=True):
        """Initialize VA_VAE
        Args:
        ...
        self.img_size = self.config.model.params.ddconfig.resolution  # Read size from config
    ...
    def encode(self, images):
        ...
        """
        with torch.no_grad():
            posterior = self.model.encode(images.to(self.device))
    ...
    def decode(self, z):
        ...
        """
        with torch.no_grad():
            images = self.model.decode(z.to(self.device))
        return images
