...
class PackDetOrderInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for i, instances_id in enumerate(results['gt_instances_ids']):
            instance = results['instances'][instances_id]
            if instance['sentence_id'] not in gt_sentence_instances:
                gt_sentence_instances[instance['sentence_id']] = {}
                gt_sentence_instances[instance['sentence_id']]['boxes'] = []
                gt_sentence_instances[instance['sentence_id']]['order'] = []
                gt_sentence_instances[instance['sentence_id']]['gt_ids'] = []
            box = results['gt_bboxes'][i].tensor[0]
            gt_sentence_instances[instance['sentence_id']]['boxes'].append(box)
            gt_sentence_instances[instance['sentence_id']]['order'].append(instance['words_order'])
            gt_sentence_instances[instance['sentence_id']]['gt_ids'].append(i)

        # 将order从小到大排序，且同时调整boxes和gt_ids
        for sentence_id in gt_sentence_instances.keys():
            order = gt_sentence_instances[sentence_id]['order']
            boxes = gt_sentence_instances[sentence_id]['boxes']
            gt_ids = gt_sentence_instances[sentence_id]['gt_ids']
            order, boxes, gt_ids = zip(*sorted(zip(order, boxes, gt_ids)))
            gt_sentence_instances[sentence_id]['order'] = list(order)
            gt_sentence_instances[sentence_id]['gt_ids'] = list(gt_ids)
            gt_sentence_instances[sentence_id]['boxes'] = list(boxes)
            sentence_box = merge_bboxes(list(boxes))
            gt_sentence_instances[sentence_id]['sentence_box'] = sentence_box

        data_sample.gt_sentence_instances = gt_sentence_instances
